...
**The file-import parsers are the newest pair, and they must agree.** `src/services/bookImport.ts` and `domain/…/book/BookImport.kt` implement the same rules for TXT / Markdown / HTML / EPUB: the 17-entry entity table, the blank-line paragraph semantics (`\n\n` between block elements — what pagination keys on), the `U+00A0` inside the whitespace-collapse class, the EPUB spine walk, and preferring the first heading over `<title>`. The Kotlin side deliberately does *not* translate the zip reader line by line: it uses `java.util.zip.ZipInputStream`, which handles data descriptors and the central directory, where the web hand-rolls that parse. Same behaviour, different plumbing — so a change to one still has to be mirrored in the other, and `BookImportTest` / `bookImport.test.ts` are the two halves of the same contract.
